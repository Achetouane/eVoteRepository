...
@section('mycontent')
    <h2 class="text-evote text-center my-3"> Gérer ma candidature </h2>
            ...
            <div class="row ">
                <div class="col-md-6 ">
                    <div class="form-group my-3 ">
                        <label for="name">Nom :</label>{{$candidat2->name}}
                    </div>
                    <div class="form-group my-3">
                        <label for="email">Email :</label>{{$candidat2->email}}
                    </div>
                    <div class="form-group my-3">
                        <label for="phone">Téléphone :</label>{{$candidat2->phone}}
                    </div>
                    <div class="form-group my-3 ">
                        <label for="partie_id">Partie :</label>{{$candidat2->partie->name}}
                    </div>
                    <div class="form-group my-3 ">
                        <label for="partie_id">ID :</label>{{$candidat2->id}}
                    </div>
                </div>
                <div class="col-md-6 ">
                    <div class="f-r form-group my-3">
                        <img src="{{ asset($candidat2->image) }}">
                    </div>
                </div>
            </div>
            ...
            <div class="container-fluid mt-2">
                <div class="table-responsive">
                    <table class="table table-striped table-hover  text-center">
                        ...
                        <tbody>
                            @foreach ($candidat2->votes as $vote)
                                <tr>
                                    <td>{{  $vote->id }}</td>
                                    <td>{{  $vote->name }}</td>
                                    ...
                                    <td>
                                        <form action="{{ route('candidat.gerer_candidature.delete', $vote->id) }}" method="post" class="d-inline">
                                            @csrf
                                            @method('delete')     
                                            <label for="candidat_id"></label>   
                                            <input type="hidden"  name="candidat_id" id="candidat_id"  value="{{ $candidat2->id}}"/>      
  ...
  <!-- Modal -->
  <form action="{{ route('candidat.gerer_candidate.candidate') }}" method="post" >
    @csrf
