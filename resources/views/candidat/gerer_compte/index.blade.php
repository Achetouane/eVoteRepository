...
@section('mycontent')
    ...
    <div class="container">
        <form action="{{ route('candidat.gerer_compte.update',  Auth::user()->id) }}" method="post">
            @csrf
            @method('put')
            ...
            <div class="form-group my-3">
                ...
                <input type="text"  name="phone" id="phone" class="form-control"  value="{{ old('phone') ?? $candidat2->phone}}"/>
           ...
           <form action="{{ route('candidat.gerer_compte.updatePassword',  Auth::user()->id) }}" method="post" >
            @csrf
            @method('put')
